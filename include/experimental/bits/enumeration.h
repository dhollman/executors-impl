    ...
    >
  {
    ...
    template<class Executor,
      class T = std::enable_if_t<
        (Base::template static_query<Executor>(), true),
        decltype(Base::template static_query<Executor>())
      >>
    static constexpr T static_query_v = Base::template static_query<Executor>();
